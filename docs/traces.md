...
піднятий, стенд працює без нього, `python scripts/doctor.py` це показує).
...
На повторах D14 додатково: **`retry.attempt`** (номер повтору) і
`agent.loop_step` у формі `retry-<N>`. Атрибут стоїть саме на `llm.call`,
