...
CAVEATS = {
    ...
    "D27": ("**Не прийнятий, і це важливо розуміти перед заняттям.** Дефект "
            "визначений як **відсутність** спана `tool.escalate_to_human`, а "
            "еталонний профіль її не гарантує: заміряно **1 пропуск на 30 "
            "прогонів** `clean` (3.3%, Wilson 95%: 0.6-16.7%) проти 10/10 на "
            "профілі. Отже одна відсутня ескалація **не доводить нічого** — "
            "ні студентові, ні вам. Показуйте як різницю частот: проженіть "
            "той самий запит 5 разів на `clean` і 5 разів на профілі. Це і є "
            "зміст класу «дефект без сліду»: інструмент вимірювання має "
            "власну похибку, і її треба знати до того, як йому вірити."),
